...
@synthesize cs,status,joy,sliders,dIn,dOut,teamNumber,ipField,myIP,IP,robotIP,controlSwitches,buttons,mode,enableDisable,goodIP,statusSmall;
...
-(id)init
{
	...
	toRobotData.packetIndex = 0x2005;
...
-(void)StartTimer
{
	...
	sender = [NSTimer scheduledTimerWithTimeInterval:.019 target:self selector:@selector(updateAndSend)
...
-(BOOL)startClient
{
	...
	memset((char *) &robot, 0, sizeof(robot));
	robot.sin_family = AF_INET;
	robot.sin_port = htons(toRobotPort);
	if (inet_aton([robotIP cStringUsingEncoding:NSASCIIStringEncoding], &robot.sin_addr)==0)
	{
...
-(BOOL)updateAndSend
{
	...
	sendto(outputSocket, &toRobotData, 1024, 0, (struct sockaddr *)&robot, sizeof(robot));
	return TRUE;
}
