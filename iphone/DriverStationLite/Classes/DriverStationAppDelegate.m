...
@synthesize scroller,sliders,sliderOutput,hide,startStop,helpPage;
...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {    
	...
	[helpPage loadRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:@"http://comets.firstobjective.org/DSLiteHelp.html"]]];
	
	[helpPage setBounds:CGRectMake(0,0,[UIScreen mainScreen].bounds.size.height, [UIScreen mainScreen].bounds.size.width)];
	
	//CGRect screen = [self.window frame];
	/*ControlScreen *cs = [[ControlScreen alloc] initWithFrame:CGRectMake(0,0,screen.size.height,screen.size.width)];
	[cs retain];
	cs.camera = YES;
	cs.opaque = NO;
	[[cam imageView] addSubview:cs];*/
...
{
	//[helpPage loadHTMLString:@"<html><head></head><body></body></html>" baseURL:nil];
